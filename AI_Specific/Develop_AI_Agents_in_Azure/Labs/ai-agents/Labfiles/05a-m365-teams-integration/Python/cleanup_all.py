...
class ResourceCleanup:
    ...
    def list_resources(self):
        ...
        try:
            ...
            for rtype, names in sorted(resource_types.items()):
                print(
                    f"   • {rtype} ({len(names)} resource{'s' if len(names) > 1 else ''})"
                )
                for name in names:
                    print(f"      - {name}")
    ...
    def cleanup_with_azd(self):
        ...
        try:
            result = subprocess.run(["azd", "down", "--force", "--purge"], check=True)
    ...
    def run(self):
        ...
        print(
            "This script will help you clean up all Azure resources created during Lab 5."
        )
        print(
