...
class PrerequisiteChecker:
    ...
    def check_command(
        self, name: str, command: List[str], min_version: str = None
    ) -> Tuple[bool, str]:
    ...
    def run_all_checks(self):
        ...
        passed, msg = self.check_command(
            "Azure Developer CLI", ["azd", "version"], min_version="1.23.0"
        )
        ...
        # Check Azure CLI
        passed, msg = self.check_command("Azure CLI", ["az", "--version"])
        self.results.append((passed, msg))
        if not passed:
            self.all_passed = False

        # Check Docker
        passed, msg = self.check_command("Docker", ["docker", "--version"])
        self.results.append((passed, msg))
        if not passed:
            self.all_passed = False
        ...
        passed, msg = self.check_command("Git", ["git", "--version"])
        self.results.append((passed, msg))
