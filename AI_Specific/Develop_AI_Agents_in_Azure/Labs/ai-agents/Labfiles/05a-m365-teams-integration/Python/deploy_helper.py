...
class DeploymentHelper:
    ...
    def azure_login(self):
        ...
        result = subprocess.run(
            ["azd", "auth", "login", "--check-status"], capture_output=True, text=True
        )
    ...
    def initialize_project(self):
        ...
        if not success:
            ...
            success = self.run_command(
                ["azd", "init"], "Initializing project", check=False
            )
    ...
    def configure_deployment(self):
        ...
        self.run_command(["azd", "env", "new", env_name], check=False)

        # Set location
        print(f"\n🔧 Setting region to '{region}'...")
        self.run_command(["azd", "env", "set", "AZURE_LOCATION", region], check=False)
    ...
    def deploy_to_azure(self):
        ...
        success = self.run_command(["azd", "up"], "Deploying to Azure")
